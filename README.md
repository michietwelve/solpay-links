...
### Umbra Protocol — $10,000 Side Track
BiePay's architecture is purpose-built to complement Umbra's stealth payment protocol. Umbra enables senders to push funds to a recipient using a stealth address that only the recipient can discover and claim. BiePay extends this model into the merchant commerce layer:

- **Stealth Merchant Links:** A BiePay payment link can be configured to route settlement to a freshly derived stealth address on every transaction, ensuring no two payments can be correlated on-chain — a critical privacy guarantee for high-value merchants.
- **One-Time Address Generation:** Each Blink generated by BiePay can include an ephemeral Umbra-compatible recipient key, making every checkout link a single-use, unlinkable payment endpoint.
- **Privacy-First Analytics:** Combined with BiePay's Incognito Mode, merchants can operate a fully private storefront where neither the payer nor an external observer can link transactions to a known merchant identity.
- **Gasless + Private:** BiePay's existing server-side fee sponsorship (Gasless Sponsorship) removes the need for the payer to hold SOL, while Umbra's stealth addressing ensures the merchant's wallet identity remains private — a uniquely powerful UX for privacy-conscious global commerce.

This dual-layer approach — Umbra for on-chain privacy and BiePay for the commerce interface — makes BiePay a direct, production-ready demonstration of Umbra's utility in real-world payment infrastructure.

**Track Link:** [Build with Umbra — $10,000 Side Track](https://superteam.fun/earn/listing/umbra-side-track)
